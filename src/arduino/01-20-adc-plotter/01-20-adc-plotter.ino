...
std::array<Sample, 1024*10> samples;
void loop() {
  // Show a count down from 5 to 0..
  for (int i = 20; i > 0; i--) {
    Serial.print(i);
    Serial.print(" ");
    delay(1000);
  ...
  // Read the ADC intil the fist measure 
  uint32_t start_time = micros();
  for(int i = 0; i < samples.size() ; i++) {
    adc_array.read(measures);
    samples[i] = {micros(), measures[0]};
  ...
  Serial.println(samples.size() * 1e6 / (end_time - start_time));
}
